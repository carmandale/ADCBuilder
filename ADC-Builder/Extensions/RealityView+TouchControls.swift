...
#else
        return simultaneousGesture(DragGesture()
            .targetedToEntity(appState.adc?.characterParent ?? Entity()) // This could be nil during app state restore.
            .onChanged { @MainActor value in
                guard let robot = appState.adc else { fatalError("No robot.") }
                ...
                appState.adc?.movementVector = SIMD3<Float>(x: 0, y: 0, z: 0)
                appState.adc?.animationState.transition(to: .walkEnd)
            }
        )
#endif
    }
