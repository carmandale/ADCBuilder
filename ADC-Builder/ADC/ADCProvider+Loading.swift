...
extension ADCProvider {
    ...
    func loadEntityFromRCPro(named entityName: String,
        ...
        do {
            let scene = try await Entity(named: sceneName, in: ADCAssetsBundle)
    ...
    func loadMaterialsFromRCPro(material: ADCMaterial) async throws -> [ADCPart: [ShaderGraphMaterial]] {
        ...
        do {
            let scene = try await Entity(named: material.sceneName, in: ADCAssetsBundle)
            for entityName in material.entityNames {
                ...
                if let part = ADCPart.partForMaterialName(name: name) {
    ...
    func loadRobotParts(taskGroup: inout TaskGroup<ADCPartLoadResult>) async {
                ...
                taskGroup.addTask {
                    do {
                        ...
                        if let entity = try await self.loadEntityFromRCPro(named: partName, fromSceneNamed: sceneName) {
                            ...
                            return ADCPartLoadResult(entity: entity, type: part, index: index)
                        } else {
    ...
    func loadRobotMaterials(taskGroup: inout TaskGroup<RobotMaterialResult>) async {
        let materials = ADCMaterial.allCases
