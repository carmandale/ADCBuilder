...
class ADC {
    ...
    init(head: Entity, body: Entity, backpack: Entity, appState: AppState, headOffset: SIMD3<Float>? = nil, backpackOffset: SIMD3<Float>? = nil) {
        self.antibody = head
        self.linker = body
        self.payload = backpack
        self.appState = appState
        
        self.linker.removeFromParent()
        self.antibody.removeFromParent()
        self.payload.removeFromParent()
        ...
        guard let skeletonClone = self.linker.findEntity(named: "rig_grp") as? ModelEntity else {
            fatalError("Didn't find expected rig_grp entity as descendent of rigged body.")
        }
        ...
        characterModel.addChild(self.antibody)
        characterModel.addChild(self.linker)
        characterModel.addChild(self.payload)
    ...
    /// Plays the robot animation for the given animation state.
        func playAnimation(_ animState: AnimationState) {
            guard let anim = linker.animationLibraryComponent?.animations[animState.rawValue] else {
                fatalError("Didn't find requested animation in library.")
            }
        if let rigGroup = linker.findEntity(named: "rig_grp") {
    ...
    func stopAllAnimations() {
        ...
        linker.stopAllAnimations(recursive: true)
    }
