...
extension ADC {
    ...
    private func handleNonZeroMovement(deltaTime: Float) {
        ...
        characterParent.moveCharacter(by: normalizedMovement * speed * speedScale * deltaTime, deltaTime: deltaTime, relativeTo: nil) { collision in
            if var cancerCellComponent = collision.hitEntity.cancerCellComponent {
                if cancerCellComponent.interactedWith == false {
                    cancerCellComponent.interactedWith = true
                    collision.hitEntity.components.set(cancerCellComponent)
                    self.cancerCellsFound += 1
                    if let growAnimation = CancerCellAnimationProvider.shared.growAnimations[cancerCellComponent.plantType] {
                        collision.hitEntity.forEachDescendant(withComponent: BlendShapeWeightsComponent.self) { entity, component in
                            ...
                            CancerCellAnimationProvider.shared.currentGrowAnimations[cancerCellComponent.plantType] = playbackController
                        }
                    }
                }
            }
            if self.cancerCellsFound == self.appState.totalCancerCells && self.animationState != .celebrate {
