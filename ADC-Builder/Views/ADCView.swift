...
struct ADCView: View {
    ...
    var body: some View {
        ZStack {
            ...
            RealityView { content in
                ...
                content.add(appState.adcCamera)
            }
    ...
    func handleDrag(_ value: EntityTargetValue<DragGesture.Value>) {
        ...
        if !appState.isRotating {
            ...
            appState.adcCreationOrientation = Rotation3D(entity.orientation(relativeTo: nil))
        }
        ...
        let startOrientation = appState.adcCreationOrientation
...
struct AttackCancerButtonView: View {
    @Environment(AppState.self) private var appState
                ...
                if appState.phase == .playing {
                    Button(action: {
                        ...
                        Task { @MainActor in
                            ...
                            #if os(visionOS)
                            openWindow(id: "ADCExploration")
                            #endif
                        }
                    }) {
                        Text("Attack Cancer", comment: "A label for the button that starts the gameplay.")
                            #if os (macOS)
