...
final public class ClubViewState: Sendable {
    ...
    func loadEnvironment() {
        ...
        if let environment = try? Entity.load(named: "scenes/volume", in: ADCAssetsBundle) {
    ...
    private func randomRobot() -> ADC? {
        let robotMaker = AppState()
        
        // Use offsets from the loaded animation rig, with some random parts
        guard let skeleton = robotMaker.adcData.meshes[.linker]?.findEntity(named: "rig_grp") as? ModelEntity else {
        ...
        guard let head = robotMaker.adcData.meshes[.antibody]?.clone(recursive: true),
              let body = robotMaker.adcData.meshes[.linker]?.clone(recursive: true),
              let backpack = robotMaker.adcData.meshes[.payload]?.clone(recursive: true) else {
            fatalError()
        }
        
        let robotCharacter = ADC(
            head: head,
            body: body,
            backpack: backpack,
            appState: robotMaker,
    ...
    private func addHostToTheCounter(_ counter: Entity) {
        ...
        robotMaker.setMesh(part: .linker, name: "body3")
