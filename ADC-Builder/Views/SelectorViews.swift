...
struct TypeSelectorView: View {
    @Binding var currentPart: ADCPart
    @Binding var meshSelection: [ADCPart: String]
    @Binding var materialSelection: [ADCPart: ADCMaterial]
...
struct MaterialColorSelectView: View {
    @Binding var currentPart: ADCPart
    @Binding var materialSelection: [ADCPart: ADCMaterial]
    @Binding var colorSelection: [ADCPart: Color]
    @Environment(AppState.self) private var appState
...
struct MaterialSelectView: View {
    ...
    var body: some View {
        VStack {
            ...
            HStack {
                ForEach(ADCMaterial.allCases) { matType in
                    Button() {
...
struct LightColorSelectView: View {
    @Binding var currentPart: ADCPart
    @Binding var lightColorSelection: [ADCPart: ADCLightColor]
    @Environment(AppState.self) private var appState
    ...
    var body: some View {
        ...
        Grid {
            GridRow {
                ForEach(ADCLightColor.allCases[0..<ADCLightColor.allCases.count / 2]) { lightColor in
                    Button() {
            ...
            GridRow {
                ForEach(ADCLightColor.allCases[ADCLightColor.allCases.count / 2..<ADCLightColor.allCases.count]) { lightColor in
                    Button() {
                        lightColorSelection[currentPart] = lightColor
                        appState.setLightColor(part: currentPart, lightColor: lightColor)
                    } label: {
