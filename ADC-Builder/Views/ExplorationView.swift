        ...
        ZStack {
            RealityView { content in
                let updateEvent = content.subscribe(to: SceneEvents.Update.self, { event in
                    ...
                    appState.adc?.handleMovement(deltaTime: deltaTime)
                })
                let animationStopEvent = content.subscribe(to: AnimationEvents.PlaybackTerminated.self, { event in
                    guard let adc = appState.adc else { return }
                    
                    if adc.cancerCellsFound == appState.totalCancerCells && !appState.celebrating {
                        for anim in CancerCellAnimationProvider.shared.currentGrowAnimations.values where anim.isPlaying {
                                return
                        }
                ...
                appState.adc?.idleTimer = 0
                #if os(visionOS)
                if appState.adc?.idleViewpoint != currentViewpoint.squareAzimuth {
                    appState.adc?.idleViewpoint = currentViewpoint.squareAzimuth
                    appState.adc?.hasChangedOrientationSinceLastWave = true
                }
                #endif
            }
